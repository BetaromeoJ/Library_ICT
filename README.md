...
- 1冊目(`book01.jpg`)には、実際の著作「先生のためのCanva教育版 やさしい入門」(技術評論社)の情報を掲載済みです。情報が変わった場合は `index.html` の該当ブロックを書き換えてください。
- 2冊目はプレースホルダーです。`index.html` 内の `<!-- 2冊目以降を追加する場合は、このカードを複製して差し替えてください -->` の下のカードを編集します。
  1. 【書籍タイトルを入力】【サブタイトルを入力】【出版社名を入力】【刊行年を入力】などの【　】部分を実際の情報に書き換える
  2. `href="【ここにAmazon等の購入ページURLを入力】"` を実際の購入ページURLに書き換える
  3. 表紙画像は `assets/images/book02.jpg` を同じファイル名で差し替える
- 3冊目以降を追加する場合は、書籍カードの `<div class="card book-card">...</div>` ブロックをコピーして追加し、`.grid-2` の中に配置する
